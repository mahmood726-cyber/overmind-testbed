...
def _project(risk="high", math_score=20, test_cmds=None, root_path="C:\\test"):
    return ProjectRecord(
        project_id="test_proj", name="TestProject",
        root_path=root_path, risk_profile=risk,
...
def test_full_pipeline_with_mocked_witnesses(tmp_path):
    engine = TruthCertEngine(baselines_dir=tmp_path)
    project = _project(risk="medium", math_score=2, root_path=str(tmp_path))
...
def test_disagreement_produces_reject(tmp_path):
    engine = TruthCertEngine(baselines_dir=tmp_path)
    project = _project(risk="high", math_score=5, root_path=str(tmp_path))  # tier 2
